...
```java
//bst的中序遍历是有序的，那我们可以考虑从中序遍历中，每次获得两个数的差值来实现
...
```java
//思路同上面 利用中序遍历有序的性质
//这样写其实是忽略了dst的属性，这里是先转为为map 又转为 list 最后转为数组。
int max_time = -1;
...
public void dfs(TreeNode root , Map<Integer,Integer> map){
    if(root == null) return;
    dfs(root.left,map);
    if(map.get(root.val)==null) map.put(root.val, 1);
    else map.put(root.val, map.get(root.val)+1);
    max_time = Math.max(max_time,map.get(root.val));
    dfs(root.right,map);
}

// 我们可以省略map的转换 直接使用中序遍历得到链表。
TreeNode pre = null;
int cur_time = -1;
int max_time = -1;
public int[] findMode(TreeNode root) {
...
public void update(TreeNode root, List<Integer> list){
    ...
    if(cur_time>max_time){
        max_time = Math.max(cur_time,max_time);
        list.clear();
        list.add(root.val);
    }
}
...
public TreeNode insertIntoBST(TreeNode root, int val) {
    ...
    if(root.val>val) {
        // 此时root的值大于目标值，且root左子树无结点，可以新增结点
        if(root.left == null){
        ...
        // 此时root的值小于目标值，且root右子树无结点，可以新增结点
        if(root.right == null){
...
public TreeNode trimBST(TreeNode root, int low, int high) {
    //终止条件一共有三个 自己为空，或者当自己的值小于最小值同时无右子树，或者当自己的值大于最大值同时无左子树。
    if(root == null) return null;
    if(root.val < low && root.right == null) return null;
    if(root.val > high && root.left == null) return null;
    
    // 当自己值小于最小值，则将右子树重新进行查找
    if(root.val < low){
        return trimBST(root.right,low,high);
    // 当自己值大于最大值，则将左子树重新进行查找
    }else if(root.val > high){
        return trimBST(root.left,low,high);
    // 当自己在这个区间，那么将左右子树重新进行查找
    }else{
...
public TreeNode trimBST(TreeNode root, int low, int high) {
    ...
    // 此时 cur 为第一个符合的点，要查找他的左子树是否符合，如果做子树不符合的话，把他左子树的右子树添加到其左子树的位置，这样就比原先的左子树大了，直到其左子树满足值大于low。更新cur为其左子树，循环。
    TreeNode cur = root;
    while(cur!=null){
