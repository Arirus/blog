## 理论基础
每-个节点由两部分组成,-个是数据域,另- 个是指针域(存放指向下—个节点的指针)最后-个节点的指针域指向NULL
...
public ListNode removeElements(ListNode head, int val) {
    ...
    ListNode next = head;
    //采用双指针 单指针也可以
    //采用单指针 不需要 next，直接说 cur.next 进行判断可以改为
    // while(cur.next!=null){
    //     if(cur.next.val == val){
...
public ListNode removeElements(ListNode head, int val) {
    if(head==null) return head;
    //注意先进行递归，不然result.next 可能是空值无法再指向新值
    head.next = removeElements(head.next, val);
    //递归完后，head.next 之后的都已经删除了，这时候再判断head的值
